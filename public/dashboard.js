...
const activityNames={
  ...
  professional_profile_submitted:'Perfil profesional enviado',professional_credentials_verify:'Credenciales profesionales verificadas',
  professional_credentials_reject:'Credenciales profesionales rechazadas',professional_credentials_suspend:'Profesional suspendido',professional_credentials_reactivate:'Profesional reactivado'
};
...
function renderItems(containerId,items,type){
  ...
  items.forEach(item=>{const card=document.createElement('article');card.className=type==='article'?'content-card blog-card':'content-card';const eyebrow=document.createElement('span');const created=type==='article'&&item.createdAt?` · ${new Intl.DateTimeFormat('es',{dateStyle:'medium'}).format(new Date(item.createdAt))}`:'';eyebrow.textContent=`${item.status==='published'?'Publicado':'Borrador'} · ${type==='article'?item.author:item.creator}${created}`;const title=document.createElement('h3');title.textContent=item.title;const copy=document.createElement('p');copy.textContent=type==='article'?item.summary:item.description;card.append(eyebrow,title,copy);if(type==='article'){const link=document.createElement('a');link.className='content-link';link.href=`/articulo.html?slug=${encodeURIComponent(item.slug)}`;link.textContent=item.pdfUrl?'Leer más y ver PDF →':'Leer más →';card.appendChild(link);if(item.pdfUrl){const pdf=document.createElement('a');pdf.className='content-link';pdf.href=item.pdfUrl;pdf.target='_blank';pdf.rel='noopener noreferrer';pdf.textContent='Descargar PDF ↗';card.appendChild(pdf);}if(['superuser','administrator'].includes(currentUser.role)||Number(item.authorId)===Number(currentUser.id)){const edit=document.createElement('button');edit.type='button';edit.className='small-button';edit.textContent='Editar artículo';edit.addEventListener('click',()=>startArticleEdit(item));card.appendChild(edit);}}else{const meta=document.createElement('div');meta.className='course-meta';meta.textContent=`${item.accessType==='paid'?'Pago':'Gratuito'} · ${Number(item.moduleCount)||0} módulos · ${Number(item.lessonCount)||0} lecciones`;card.appendChild(meta);const hint=document.createElement('p');hint.className='course-admin-hint';hint.textContent=item.accessType==='paid'&&!item.prices?.length?'Añade un precio para ofrecer este curso.':!item.lessonCount?'Añade lecciones para mostrar el temario.':item.enrollmentPolicy==='admin_only'?'La inscripción requiere asignación administrativa.':'Inscripción disponible según la política del curso.';card.appendChild(hint);if(['superuser','administrator'].includes(currentUser.role)||Number(item.creatorId)===Number(currentUser.id)){const edit=document.createElement('button');edit.type='button';edit.className='small-button';edit.textContent='Editar curso';edit.addEventListener('click',()=>startCourseEdit(item));const content=document.createElement('button');content.type='button';content.className='small-button';content.textContent='Editar contenido';content.addEventListener('click',()=>loadCourseContentEditor(item));card.append(edit,content);}}container.appendChild(card);});
}
...
let managedCourses=[];
function filterManagedCourses(){const search=document.querySelector('#admin-course-search').value.trim().toLocaleLowerCase('es');const status=document.querySelector('#admin-course-status').value;const filtered=managedCourses.filter(course=>(!status||course.status===status)&&(!search||`${course.title} ${course.description}`.toLocaleLowerCase('es').includes(search)));renderItems('#courses-list',filtered,'course');if(!filtered.length&&managedCourses.length)document.querySelector('#courses-list .empty-state').textContent='No hay cursos que coincidan con los filtros.';}
async function loadContent(){const [{articles},{courses}]=await Promise.all([request('/api/content/articles'),request('/api/content/courses')]);managedCourses=courses;renderItems('#articles-list',articles,'article');filterManagedCourses();renderRoleSummary(articles,courses);document.querySelectorAll('.course-selector').forEach(select=>{select.textContent='';courses.filter(course=>['superuser','administrator'].includes(currentUser.role)||course.creatorId===currentUser.id).forEach(course=>{const option=document.createElement('option');option.value=course.id;option.textContent=course.title;select.appendChild(option);});});}
...
async function loadUsers(reset=true){
  const body=document.querySelector('#users-body');const more=document.querySelector('#users-more');
  if(reset){usersCursor=null;body.textContent='';}
  const query=new URLSearchParams({limit:'50'});const search=document.querySelector('#user-search').value.trim();if(search)query.set('search',search);if(usersCursor)query.set('cursor',usersCursor);
  const {users,nextCursor}=await request(`/api/users?${query}`);
  if(!users.length&&!body.children.length){const row=document.createElement('tr');const cell=document.createElement('td');cell.colSpan=5;cell.className='empty-state';cell.textContent='No hay usuarios que coincidan con la búsqueda.';row.appendChild(cell);body.appendChild(row);}
  ...
  body.querySelectorAll('[data-role-id]').forEach(control=>{if(!control.dataset.bound){control.dataset.bound='true';control.addEventListener('change',()=>updateUser(control.dataset.roleId,'role',control.value));}});
  body.querySelectorAll('[data-status-id]').forEach(button=>{if(!button.dataset.bound){button.dataset.bound='true';button.addEventListener('click',()=>updateUser(button.dataset.statusId,'status',button.dataset.status));}});
  usersCursor=nextCursor;more.hidden=!nextCursor;
}
...
const professionalTypeNames={psychologist:'Psicólogo/a',psychiatrist:'Psiquiatra',counselor:'Consejero/a u orientador/a'};
const professionalServiceTypes={psychologist:'psychology',psychiatrist:'psychiatry',counselor:'counseling'};
const weekdayNames={mon:'Lunes',tue:'Martes',wed:'Miércoles',thu:'Jueves',fri:'Viernes',sat:'Sábado',sun:'Domingo'};
async function loadProfessionals(){
  const{professionals,candidates}=await request('/api/scheduling/professionals');const list=document.querySelector('#professionals-list');const select=document.querySelector('#professional-user');list.textContent='';select.textContent='';
  candidates.forEach(user=>{const option=document.createElement('option');option.value=user.id;option.textContent=`${user.fullName} · ${user.email}`;select.appendChild(option);});
  document.querySelector('#professional-form').querySelector('button[type="submit"]').disabled=!candidates.length;
  const search=document.querySelector('#professional-search').value.trim().toLocaleLowerCase('es');
  const filtered=professionals.filter(professional=>[professional.fullName,professional.email,professional.specialties,professional.licenseNumber].some(value=>String(value||'').toLocaleLowerCase('es').includes(search)));
  if(!filtered.length){const empty=document.createElement('p');empty.className='empty-state';empty.textContent=search?'No hay profesionales que coincidan con la búsqueda.':'No hay perfiles profesionales.';list.appendChild(empty);return;}
  filtered.forEach(professional=>list.appendChild(professionalCard(professional)));
}
function professionalCard(professional){
  const card=document.createElement('article');card.className='application-review';const heading=document.createElement('div');heading.className='application-person';const state=document.createElement('span');state.textContent=`${professional.credentialStatus} · ${professional.status}`;const title=document.createElement('h3');title.textContent=professional.fullName;const detail=document.createElement('p');detail.textContent=`${professionalTypeNames[professional.professionalType]} · ${professional.email} · ${professional.timezone}`;const license=document.createElement('p');license.textContent=`Matrícula/licencia: ${professional.licenseNumber||'No indicada'}`;heading.append(state,title,detail,license);card.appendChild(heading);
  if(professional.verificationNotes){const notes=document.createElement('p');notes.textContent=`Última revisión: ${professional.verificationNotes}`;card.appendChild(notes);}
  const review=document.createElement('form');review.className='editor-form';review.innerHTML='<div class="form-grid"><div class="field"><label>Decisión</label><select name="decision"><option value="verify">Verificar y habilitar</option><option value="reject">Rechazar credenciales</option><option value="suspend">Suspender</option><option value="reactivate">Reactivar</option></select></div><div class="field"><label>Fundamento</label><textarea name="notes" minlength="5" maxlength="2000" required></textarea></div></div><button class="small-button" type="submit">Registrar decisión</button><p class="form-message"></p>';
  review.addEventListener('submit',async event=>{event.preventDefault();const message=review.querySelector('.form-message');try{const data=await request(`/api/scheduling/professionals/${professional.id}/credentials`,{method:'PATCH',body:JSON.stringify(Object.fromEntries(new FormData(review)))});message.className='form-message success';message.textContent=data.message;await loadProfessionals();}catch(error){message.className='form-message error';message.textContent=error.message;}});card.appendChild(review);
  const existing=document.createElement('div');existing.className='application-answer';const serviceSummary=document.createElement('p');serviceSummary.textContent=`Servicios: ${professional.services.map(item=>item.name).join(', ')||'ninguno'}`;const scheduleSummary=document.createElement('p');scheduleSummary.textContent=`Disponibilidad: ${professional.availability.map(item=>`${weekdayNames[item.weekday]} ${String(item.startTime).slice(0,5)}–${String(item.endTime).slice(0,5)}`).join(', ')||'ninguna'}`;existing.append(serviceSummary,scheduleSummary);card.appendChild(existing);
  if(professional.status==='active'&&professional.credentialStatus==='verified'){
    const service=document.createElement('form');service.className='editor-form';service.innerHTML='<h4>Crear servicio y precio</h4><div class="form-grid"><div class="field"><label>Nombre</label><input name="name" minlength="5" maxlength="180" required></div><div class="field"><label>Duración (minutos)</label><input name="durationMinutes" type="number" min="15" max="240" value="50" required></div><div class="field"><label>Moneda</label><select name="currency"><option value="ARS">ARS · Mercado Pago</option><option value="USD">USD · PayPal</option></select></div><div class="field"><label>Importe</label><input name="amount" inputmode="decimal" pattern="\\d+(\\.\\d{1,2})?" required></div></div><div class="field"><label>Descripción</label><textarea name="description" minlength="20" maxlength="10000" required></textarea></div><button class="small-button" type="submit">Crear servicio</button><p class="form-message"></p>';
    service.addEventListener('submit',async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(service));const message=service.querySelector('.form-message');try{const created=await request(`/api/scheduling/professionals/${professional.id}/services`,{method:'POST',body:JSON.stringify({serviceType:professionalServiceTypes[professional.professionalType],name:values.name,description:values.description,durationMinutes:Number(values.durationMinutes)})});await request(`/api/commerce/services/${created.id}/prices`,{method:'POST',body:JSON.stringify({currency:values.currency,amountMinor:Math.round(Number(values.amount)*100)})});message.className='form-message success';message.textContent='Servicio y precio creados.';service.reset();await loadProfessionals();}catch(error){message.className='form-message error';message.textContent=error.message;}});card.appendChild(service);
    const availability=document.createElement('form');availability.className='editor-form';availability.innerHTML='<h4>Agregar disponibilidad semanal</h4><div class="form-grid"><div class="field"><label>Día</label><select name="weekday"><option value="mon">Lunes</option><option value="tue">Martes</option><option value="wed">Miércoles</option><option value="thu">Jueves</option><option value="fri">Viernes</option><option value="sat">Sábado</option><option value="sun">Domingo</option></select></div><div class="field"><label>Desde</label><input name="startTime" type="time" required></div><div class="field"><label>Hasta</label><input name="endTime" type="time" required></div></div><button class="small-button" type="submit">Agregar horario</button><p class="form-message"></p>';
    availability.addEventListener('submit',async event=>{event.preventDefault();const message=availability.querySelector('.form-message');try{const data=await request(`/api/scheduling/professionals/${professional.id}/availability`,{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(availability)))});message.className='form-message success';message.textContent=data.message;availability.reset();await loadProfessionals();}catch(error){message.className='form-message error';message.textContent=error.message;}});card.appendChild(availability);
  }
  return card;
}
function setupProfessionalManagement(){
  const form=document.querySelector('#professional-form');form.addEventListener('submit',async event=>{event.preventDefault();const message=form.querySelector('.form-message');try{const data=await request('/api/scheduling/professionals',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(form)))});message.className='form-message success';message.textContent=data.message;form.reset();form.elements.timezone.value='America/Panama';await loadProfessionals();}catch(error){message.className='form-message error';message.textContent=error.message;}});
}
...
document.querySelector('#user-filters').addEventListener('submit',event=>{event.preventDefault();loadUsers();});
document.querySelector('#users-clear').addEventListener('click',()=>{document.querySelector('#user-filters').reset();loadUsers();});
document.querySelector('#users-more').addEventListener('click',()=>loadUsers(false));
let professionalSearchTimer;
document.querySelector('#professional-search').addEventListener('input',()=>{clearTimeout(professionalSearchTimer);professionalSearchTimer=setTimeout(()=>loadProfessionals(),250);});
setupNavigation();
document.querySelector('#admin-course-search').addEventListener('input',filterManagedCourses);
document.querySelector('#admin-course-status').addEventListener('change',filterManagedCourses);
document.querySelector('#admin-new-course').addEventListener('click',()=>{resetCourseEditor();document.querySelector('#course-form').scrollIntoView({behavior:'smooth',block:'start'});document.querySelector('#course-title').focus();});
init();
