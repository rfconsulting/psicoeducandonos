...
form.addEventListener('submit',async event=>{
  ...
  try{if(!csrfToken)await getCsrf();const response=await fetch('/api/applications',{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json','x-csrf-token':csrfToken},body:JSON.stringify(values)});const data=await response.json();if(!response.ok)throw new Error(data.error||'No fue posible enviar la postulación.');message.classList.add('success');message.textContent=data.message;form.reset();feedbackField.hidden=true;if(window.NotificationModal)window.NotificationModal.show({type:'success',title:'¡Postulación enviada!',message:'Usted se ha postulado exitosamente.',buttonLabel:'Entendido'});else window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'});}catch(error){message.classList.add('error');message.textContent=error.message;csrfToken='';}finally{button.disabled=false;}
});
getCsrf().catch(()=>{});
